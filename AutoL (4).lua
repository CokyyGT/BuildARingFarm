...
-- Disconnect old event connections (prevent double trigger)
_G.AutoRollConnections = _G.AutoRollConnections or {}
for _, conn in ipairs(_G.AutoRollConnections) do
    pcall(function() conn:Disconnect() end)
end
_G.AutoRollConnections = {}
...
--         CONFIG SAVE/LOAD
-- ══════════════════════════════════════
local CONFIG_FILE = "autoroll_l_config.json"
local WEBHOOK_CONFIG_FILE = "autoroll_l_webhook.json"
...
local function saveWebhookConfig()
    pcall(function()
        writefile(WEBHOOK_CONFIG_FILE, HttpService:JSONEncode({webhook_url = WEBHOOK_URL, webhook_enabled = WEBHOOK_ENABLED}))
    end)
end
...
local function loadWebhookConfig()
    local ok, raw = pcall(function() return readfile("autoroll_l_webhook.json") end)
    if ok and raw then
...
local function saveWebhookConfig()
    pcall(function() 
        writefile("autoroll_l_webhook.json", HttpService:JSONEncode({webhook_url = WEBHOOK_URL}))
    end)
end
...
local function loadRunState()
    local ok, raw = pcall(function() return readfile("autoroll_l_run_state.json") end)
    if ok and raw then
        local ok2, data = pcall(function() return HttpService:JSONDecode(raw) end)
...
local function saveRunState()
    pcall(function() 
        writefile("autoroll_l_run_state.json", HttpService:JSONEncode({
            running = Running,
...
local HourlyOther  = {}
local HourlyTotal  = 0
local HourlyRarity = {} -- Track rarity yang dibeli
    local HourlyRarityRolled = {} -- Track rarity yang muncul (rolled)   -- tracking rarity yang muncul saat roll
                ...
                if lv then
                    ...
                    if not rollPrompt then
                        for _, d in ipairs(lv:GetDescendants()) do
                            if d:IsA("ProximityPrompt") and d.Name == "RollSeeds" then 
                                rollPrompt = d; break 
                            end
                        end
                    end
                    -- Last resort: cari prompt dengan ActionText "Pull Lever"
                    if not rollPrompt then
                        for _, d in ipairs(lv:GetDescendants()) do
                            if d:IsA("ProximityPrompt") and d.ActionText and d.ActionText:find("Lever") then 
                                rollPrompt = d; break 
                            end
                        end
                    end
            ...
            if not rollPrompt then
                for _, d in ipairs(plot:GetDescendants()) do
                    if d:IsA("ProximityPrompt") and
                       (d.Name:match("Roll") or d.Name:match("Seed")) and
                       not d.Name:match("AFK") then
                        rollPrompt = d; break
                    end
                end
            end
...
--           DRAG
-- ══════════════════════════════════════
-- Drag header (Mobile/Delta touch compatible)
local dragging = false
local dragStart = nil
local startPos = nil

local dragInputBegin = header.InputBegan:Connect(function(inp)
...
table.insert(_G.AutoRollConnections, dragInputBegin)
...
local function RunLoop()
    ...
    -- Jackpot = tier tertinggi setelah 16x
    local BONUS_TIERS = {
        ["2xClover"] = 1,
        ["4xClover"] = 2,
        ["8xClover"] = 3,
    ...
    while Running do
        ...
        -- Roll
        if not rollPrompt then
            addLog("❌ rollPrompt not found!", C.red)
            Running = false
            break
        end
        local ok, err = FirePrompt(rollPrompt)
        if ok then
            TotalRolls = TotalRolls + 1
            addLog("✅ Roll fired", C.green)
...
if Running and not _G.AutoRollResumed then
    ...
    task.delay(0.5, function()
        -- IMPORTANT: Re-find plot & prompt saat resume (jangan pakai cache lama)
        myPlot = nil
        rollPrompt = nil
        if not FindPlotAndPrompt() then
            addLog("❌ Plot/RollSeeds tidak ditemukan saat resume!", C.red)
            Running = false
            runBtn.Text = "▶  Mulai Auto Roll"
            runBtn.BackgroundColor3 = C.green
            return
        ...
        addLog("✅ Plot found, resuming auto roll...", C.gold)
        task.spawn(RunLoop)
    end)
end
