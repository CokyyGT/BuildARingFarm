...
local function SendRareAlert(itemName, amount)
    ...
    for keyword, role in pairs(RARE_ITEMS) do
        if lower:find(keyword:lower()) then mention = role break end
    end
...
local function SendHourlyReport()
    ...
    LastCrateMoney = crate  -- Track for potential future use
    ...
    table.insert(sections, "**⚡ Cash/min:** `"..cashMin.."`  •  **💵 Est/hour:** `"..estHour.."`")
    table.insert(sections, "**🎲 Total Roll:** `"..TotalRolls.."`  •  **🌱 Bought:** `"..TotalBought.."`")
    ...
    DoRequest({ embeds = {{
        title = "📊 1-Hour Report — Build A Ring Farm",
        description = table.concat(sections, "\n"),
        color = 16750592,
...
local function SendActivated()
    ...
    DoRequest({ embeds = {{
        ...
        color = 3066993,
        footer = { text = "Credit:@Coky  •  Auto Roll" },
        timestamp = DateTime.now():ToIsoDate()
    }}})
end
...
local originalColor = main.BackgroundColor3

-- Check if touch di header area
local function isTouchingHeader(pos)
    local h = header.AbsolutePosition
    local s = header.AbsoluteSize
    return pos.X >= h.X and pos.X <= h.X + s.X and
           pos.Y >= h.Y and pos.Y <= h.Y + s.Y
end
...
UIS.InputBegan:Connect(function(inp, gameProc)
    ...
    if (inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch) and isTouchingHeader(inp.Position) then
        ...
        main.BackgroundColor3 = Color3.fromRGB(255, 200, 0)  -- Kuning
    end
end)
...
UIS.InputEnded:Connect(function(inp, gameProc)
    if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
        dragging = false
        main.BackgroundColor3 = originalColor  -- Normal
    end
end)
...
local function RunLoop()
    ...
    addLog("Auto roll Started!", C.accent)
    SendActivated()
